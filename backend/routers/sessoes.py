...
class PreTreinoInput(BaseModel):
    ...
    temp_celsius: Optional[float] = Field(
        default=None,
        ge=-10,
        le=60
    )

    umidade_pct: Optional[float] = Field(
        default=None,
        ge=0,
        le=100
    )

    cor_urina_basal: Optional[int] = Field(
        default=None,
        ge=1,
        le=8
    )
...
def finalizar_sessao(
    ...
    sessao.tempo_total_segundos = body.get("tempo_total_segundos")
    sessao.ingestao_ml = body.get("ingestao_ml")
    sessao.volume_urina_ml = body.get("volume_urina_ml")
...
def registrar_fluido(
    ...
    sessao.ingestao_ml = (
        sessao.ingestao_ml or 0
    ) + volume_ml

    db.commit()

    return {
        "ingestao_total_ml": sessao.ingestao_ml
    }
...
async def finalizar_pos_treino(
    ...
    sessao.condicao_vestimenta = (
        body.condicao_vestimenta
    )
    ...
    sessao.taxa_sudorese = (
        resultado["taxa_sudorese"]
    )

    sessao.variacao_peso_pct = (
        resultado["variacao_peso_pct"]
    )
    ...
    recom = RecomendacaoIA(
        ...
        ingestao_recomendada_ml_h=rec[
            "ingestao_recomendada_ml_h"
        ],
        intervalo_minutos=rec[
            "intervalo_minutos"
        ],
    )
    ...
    return {
        ...
        "dados_recebidos": body.model_dump(),
...
def detalhe_sessao(
    ...
    return {
        ...
        "recomendacao": {
            "texto": rec.texto,
            "ingestao_recomendada_ml_h":
                rec.ingestao_recomendada_ml_h,

            "intervalo_minutos":
                rec.intervalo_minutos,
...
def _agregar_por_mes(sessoes: list) -> list:
    ...
    return [
        {
            "mes": k,
            "taxa_media": round(sum(v) / len(v), 2)
        }
