...
def calcular_taxa_sudorese(
    ...
    if not peso_pre or not peso_pos or peso_pre <= 0 or peso_pos <= 0:
    ...
    if duracao_minutos <= 5:
        variacao_simples = abs(peso_pre - peso_pos) / peso_pre * 100
        return {
            "taxa_sudorese":    1.0,
            "variacao_peso_pct": round(variacao_simples, 2),
            "perda_liquida_kg": round(abs(peso_pre - peso_pos), 3),
    ...
    balanco_fluido = (ingestao_ml - urina_ml) / 1000

    perda_liquida = perda_kg + balanco_fluido

    fator = 1 + FATOR_VESTIMENTA.get(condicao_vestimenta, 0.2)
    perda_ajustada = max(0.0, perda_liquida * fator)   # nunca negativo

    taxa_l_h = perda_ajustada / duracao_h
...
def gerar_recomendacao(
    taxa_l_h: float,
    variacao_pct: float,
    duracao_segundos: Optional[float] = None,   # CORREÇÃO: agora opciona
) -> dict:
    ...
    taxa_l_h = max(0.1, min(float(taxa_l_h), 2.5))
