...
def _font(bold=False, size=10, color="1a1a18", italic=False) -> Font:
    return Font(bold=bold, size=size, color=color, italic=italic,
                name="Calibri")


def _border(all_sides=False) -> Border:
...
def _aplicar_header_row(ws, row: int, colunas: list[str]):
...
def _aba_historico(wb: Workbook, sessoes: list[dict], atleta: dict):
    ws = wb.active
    ws.title = "Histórico"
    ws.freeze_panes = "A3"
    ...
    title_cell.fill      = _fill(COR_DARK)
    title_cell.font      = _font(bold=True, color=COR_WHITE, size=12)
...
def _aba_grafico(wb: Workbook, sessoes: list[dict]):
    ...
    sessoes_ordenadas = sorted(
        [s for s in sessoes if s.get("taxa_sudorese_lh")],
        key=lambda s: s.get("criada_em") or s.get("data") or "",
    )
    ...
    series.graphicalProperties.line.solidFill  = COR_PRIMARIA
    series.graphicalProperties.line.width      = 20000
    series.marker.symbol  = "circle"
    series.marker.size    = 6
    series.marker.graphicalProperties.fgColor  = COR_ACCENT
    series.marker.graphicalProperties.solidFill = COR_ACCENT
...
def gerar_excel_historico(sessoes: list[dict], atleta: dict) -> bytes:
    ...
    Args:
        ...
        atleta:  dict com codigo_anonimizado
